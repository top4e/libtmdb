...
#include "company_movies.h"

static int company_movies_parse(jsmntok_t	*tok,
				size_t		cnt,
				char		*data,
				void		*udata)
{
				  ...
				  const char *page)
{
	...
	ctx.request_type.type = COMPANY_MOVIES;
	if (company_id)
	...
	if (!result) {
		header_process(&data);
		body_process(&data, COMPANY_MOVIES_NUM_TOKENS,
			     company_movies_parse, &movies);
	}
