...
#include "movie_top_rated.h"

static int movie_top_rated_parse(jsmntok_t	*tok,
				 size_t		cnt,
				 char		*data,
				 void		*udata)
{
...
tmdb_movie_t *tmdb_movie_top_rated(const char *language)
{
	...
	ctx.request_type.type = MOVIE_TOP_RATED;
	if (language)
	...
	if (!result) {
		header_process(&data);
		body_process(&data, MOVIE_TOP_RATED_NUM_TOKENS,
			     movie_top_rated_parse, &movies);
	}
