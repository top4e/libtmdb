...
#include "movie_popular.h"

static int movie_popular_parse(jsmntok_t	*tok,
			       size_t		cnt,
			       char		*data,
			       void		*udata)
{
...
tmdb_movie_t *tmdb_movie_popular(const char *language)
{
	...
	ctx.request_type.type = MOVIE_POPULAR;
	if (language)
	...
	if (!result) {
		header_process(&data);
		body_process(&data, MOVIE_POPULAR_NUM_TOKENS,
			     movie_popular_parse, &movies);
	}
