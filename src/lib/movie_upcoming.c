...
#include "movie_upcoming.h"

static int movie_upcoming_parse(jsmntok_t	*tok,
				size_t		cnt,
				char		*data,
				void		*udata)
{
...
tmdb_movie_t *tmdb_movie_upcoming(const char *language)
{
	...
	ctx.request_type.type = MOVIE_UPCOMING;
	if (language)
	...
	if (!result) {
		header_process(&data);
		body_process(&data, MOVIE_UPCOMING_NUM_TOKENS,
			     movie_upcoming_parse, &movies);
	}
