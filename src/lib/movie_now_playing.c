...
#include "movie_now_playing.h"

static int movie_now_playing_parse(jsmntok_t	*tok,
				   size_t	cnt,
				   char		*data,
				   void		*udata)
{
...
tmdb_movie_t *tmdb_movie_now_playing(const char *language)
{
	...
	ctx.request_type.type = MOVIE_NOW_PLAYING;
	if (language)
	...
	if (!result) {
		header_process(&data);
		body_process(&data, MOVIE_NOW_PLAYING_NUM_TOKENS,
			     movie_now_playing_parse, &movies);
	}
